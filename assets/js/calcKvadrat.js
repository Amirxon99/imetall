...
function calcView(){
  if(lang2=='uz'){
    ...
    calculatorName.textContent="Квадрат";
    ...
    result.textContent = result.textContent.slice(0, -2) + " м"; 
      ...
      labelDiametr.textContent="Сторона , мм "
      labelLength.textContent = "Длина, м.";
      lengthOfArmature.value = "1";
      result.parentNode.children[2].textContent = "Вес КВАДРАТА: кг";
...
function calculatorArmature(d, l, active) {
  ...
  // }
  if (active == "length") {
    const m = p*d*d*l*0.000001;
    result.textContent = m.toFixed(3) + " kg";
  } else {
    const m = l / (p * d * d * 0.000001);
